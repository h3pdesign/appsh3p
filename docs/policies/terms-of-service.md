# Terms of Service

_Last updated: February 24, 2026_

These Terms of Service govern use of the h3p apps documentation website at https://apps-h3p.com.

## 1. Scope

This website provides documentation, guides, release notes, and support references for h3p apps.

These terms also apply to your use of h3p-provided apps referenced on this site, including:

- Neon Vision Editor
- Metrics Data
- Release Assistant
- X-Newsbook

Use of each app must follow applicable platform terms (for example Apple App Store or TestFlight terms) in addition to these terms.

## 2. Acceptable use

You agree not to:

1. misuse, probe, or disrupt the service
2. attempt unauthorized access
3. copy or republish content in a misleading way
4. use the site for unlawful activity

## 3. Intellectual property

Unless stated otherwise, content, design, and branding are owned by h3p Hilthart Pedersen.

## 4. No warranty

The site is provided "as is" and may change without notice. Documentation may contain errors or omissions.

## 5. External links

Links to third-party services (for example GitHub, Apple, Patreon, TestFlight) are provided for convenience. Their terms and privacy rules apply separately.

## 6. Liability

To the maximum extent permitted by law, h3p Hilthart Pedersen is not liable for indirect or consequential damages from use of this website.

For h3p apps, this includes damages resulting from data loss, workflow interruption, analytics/reporting differences, or third-party API/provider changes.

## 7. Changes

These terms may be updated. Material changes are reflected by the "Last updated" date.

## 8. Contact

For support and legal requests, use [Support and Feedback](/support/support-and-feedback).
