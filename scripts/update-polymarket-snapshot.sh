...
const ids = [...new Set(entries.map(entry => String(entry?.market_id || '').trim()).filter(Boolean))]
...
while IFS= read -r ID; do
  [ -n "$ID" ] || continue
  BODY=$(curl -L -sS --max-time 15 -H "accept: application/json" -H "user-agent: $USER_AGENT" "$API_BASE/$ID" || true)

  if [ -n "$BODY" ] && printf '%s' "$BODY" | node -e "const fs=require('fs');const raw=fs.readFileSync(0,'utf8');const p=JSON.parse(raw);if(!p||!p.id)process.exit(1);" 2>/dev/null; then
    printf '%s\n' "$BODY" >> "$SUCCESS_FILE"
  else
...
node - <<'NODE' "$SUCCESS_FILE" "$FAILED_FILE" "$OUT_PATH" "$TOTAL_IDS"
const fs = require('fs')

const [successPath, failedPath, outPath, totalIdsRaw] = process.argv.slice(2)
